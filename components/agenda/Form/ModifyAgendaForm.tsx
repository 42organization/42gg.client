...
const ModifyAgendaForm = ({ data, handleSubmit }: ModifyAgendaFormProps) => {
  ...
  useEffect(() => {
    if (data) {
      setRecruitEnd(new Date(data.agendaDeadLine));
      setDateRange([
        new Date(data.agendaStartTime),
        new Date(data.agendaEndTime),
      ]);
      setTeamLimit([data.agendaMinTeam, data.agendaMaxTeam]);
      setPeopleLimit([data.agendaMinPeople, data.agendaMaxPeople]);
  ...
  return (
    ...
    >
      <TitleInput
        ...
        defaultValue={data.agendaTitle}
      />
      <DescriptionInput
        name='agendaContent'
        label='설명'
        placeholder='설명을 입력해주세요'
        defaultValue={data.agendaContent}
      />
